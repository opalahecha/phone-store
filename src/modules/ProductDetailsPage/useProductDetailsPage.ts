...
export const useProductDetailsPage = () => {
  ...
  const hideDiscount = Boolean(
    (state as ProductDetailsLocationState | null)?.hideDiscount,
  );
  ...
  const product = useMemo<Product | undefined>(() => {
    ...
    return products.find(
      item => (item.itemId ?? String(item.id)) === productId,
    );
  }, [products, productId]);
  ...
  const currentItemId = String(
    product?.itemId ?? display?.itemId ?? display?.id ?? '',
  );
  const cartProductBase = currentItemId
    ? (products.find(item => item.itemId === currentItemId) ?? product)
    : product;
  const { isInCart, isFavorited, handleCartClick, handleFavoriteClick } =
    useProductActions({
  ...
  useEffect(() => {
    ...
    setSelectedColor(
      display.colorsAvailable?.[0] ??
        (display.color ? String(display.color) : ''),
    );
    setSelectedCapacity(
      display.capacityAvailable?.[0] ?? display.capacity ?? '',
    );
  ...
  const variantProducts = useMemo(() => {
    ...
    return products.filter(
      item =>
        ...
        getProductNamespace(item) === namespace,
    );
  ...
  const findVariant = useCallback(
    (capacity: string | number, color: string) =>
      variantProducts.find(
        item =>
          normalizeOption(getProductCapacity(item)) ===
            normalizeOption(capacity) &&
